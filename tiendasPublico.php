...
<?php
  session_start();   // crea o reanuda una sesion
   if (!isset($_SESSION['viveresMarisol']))  // si no hay nada en la variable superglobal llamada carrito (ver el signo !)
        {
          $_SESSION['viveresMarisol']= [];  //la declaramos una sesion llamada viveres Marisol
          // declaramos una variable de la sesion viveres marisol que llamaremos categoria y le asignamos el id recibido de la pagina index
         
          $_SESSION['viveresMarisol']['categoria']=1;
          $_SESSION['viveresMarisol']['carrito']=0;
         }
         
          else {
              if(!isset($_SESSION['viveresMarisol']['categoria']))
              {
               $_SESSION['viveresMarisol']['categoria']=1;   
              }         
             echo"<script>";echo"rellenarCarrito();";echo"</script>";
             }
...
<head>
  <meta charset="UTF-8">
  <title>Viveres Marisol Articulos</title>
 ...
 <div class="main">
     ...
     <?php include("tablaCompras.php"); ?>
       ...
       <div id="php">
                        ...
                        <?php
                             $id=$_SESSION['viveresMarisol']['categoria']; 
                             require_once("controladores/funcionesAction.php");
                              $tiendas=new Controladores();    
                              $listaCategorias=$tiendas->obtener_Categorias();
                              $listaArticulos=$tiendas->obtener_Articulos($id);
                              //  imprime todo el arrays 
                              //   echo "<pre>";
                              //     print_r($listaArticulos);
                               //      echo "</pre>";
                              $listaArticulosTodos=$tiendas->obtener_Articulos_Todos();
                              // obtenemos de la lista articulos una lista con todos los nombres de los articulos
                               $arrayNombres = array_column($listaArticulosTodos, 'nombre_Art');
                              //  echo "<pre>";
...
?>       
            <?php foreach ($listaCategorias as $categorias): ?> 
                 ...
                 <div class="contenedorCategorias">
                     <div>
                         <button  class="<?php echo $categorias['id_Categoria']?>" onclick="cambiarCategoria(<?php echo $categorias['id_Categoria']?>)" ><img  src="imagenes/categorias/<?php echo $categorias['id_Categoria']?>.jpg?version=<?php echo $version?>" alt=""></button>
       ...
       <div class="elementosFijos">  
         <div class="aside">
           ...
           <div class="selector">
               ...
               <input id="tag" placeholder="Buscar Articulo">
    ...
    $.get("controladores/obtenerArticuloAction.php", params, function (response) {
       ...
       <div class="container"> 
         ...
         <article>
                     ...
                     <?php foreach ($listaArticulos as $elemento):?>   
                          <div class="articulo">
                            
                            <div class="foto" style="position: relative; left: 0; top: 0;">
                             <!--    <img src="imagenes/articulos/<?php echo $elemento['id_Art']?>.jpg?version=<?php echo $version;?>" alt=""> -->
                                 <img src="imagenes/articulos/<?php echo $elemento['id_Art']?>.jpg?version=<?php echo $version?>" alt="">
                             </div>
                             <div class="informacion">
                                 <div id="idArticulo" hidden="">
                                     <?php  echo $elemento['id_Art']?>
                                 </div>
                                 <div id="detalles">
                                     
                                     <span> <?php echo $elemento['nombre_Art']?></span> 
                                     
                                     
                                  </div>
                                 <span id="descripcion" hidden=""><?php echo $elemento['descripcion_Art']?></span>
                                  <span class="separador">  ***** </span>
                                  <span id="precio"> <?php echo $elemento['precio_Art']?> $</span>   
                             </div>
                             <div id="compras">
                                 <button type="button"  id="botonCarrito"  onclick="agregarArticulo(<?php  echo $elemento['id_Art']?>,'<?php echo $elemento['nombre_Art']?> <?php echo $elemento['descripcion_Art']?>','<?php echo $elemento['precio_Art']?>')"><span class="ico"><img id="imagen1" border="0" src="imagenes/iconos-svg/carroComprasNaranja.svg" width="20" height="20" ><img id="imagen2" border="0" src="imagenes/iconos-svg/carroComprasBlanco.svg" width="20" height="20" ></span>&nbsp AGREGAR AL CARRITO</button>
                             </div>
                        </div> 
